...
def add_ball(space):
    ...
    y = random.randint(200 + bodyheight, 250 + bodyheight)
    body.position = x, y
...
def add_hourglass(space):
    ...
    shape2 = pymunk.Poly(bodyrighttriangle, [(0, 0), (0, 50 + angle), (-50, 50)])
...
def printballdata(ballx, balltime):
    ...
    xprime = np.array(balltime)
    yprime = np.array(ballx)
    y_spl = UnivariateSpline(xprime, yprime, s=0, k=4)
    x_range = np.linspace(xprime[0], xprime[-1], 1000)
...
def selTA():
    abdistance = math.sqrt(((0-50)**2)+((0-50)**2))
    acdistance = math.sqrt(((0-0)**2)+((0-(50+triangleAngle.get()))**2))
...
scalePartSize = Scale(framePartSize, variable=particleSize, from_=1, to=10, orient=HORIZONTAL, length=300)
buttonPartSizeValue = Button(framePartSize, text="Get Value", command=selPS)
labelPartSizeValue = Label(framePartSize, text="Value = 1.0")
scalePartSize.set(1)
...
labelPartMassValue = Label(framePartMass, text="Value = 100.0")
scalePartMass.set(100)
...
scalePartAmt = Scale(framePartAmt, variable=particleAmount, from_=1, to=5000, orient=HORIZONTAL, length=300)
buttonPartAmtValue = Button(framePartAmt, text="Get Value", command=selPA)
labelPartAmtValue = Label(framePartAmt, text="Value = 500")
scalePartAmt.set(500)
...
scaleFunHeight = Scale(frameFunHeight, variable=funnelHeight, from_=10, to=200, orient=HORIZONTAL, length=300)
buttonFunHeightValue = Button(frameFunHeight, text="Get Value", command=selFH)
labelFunHeightValue = Label(frameFunHeight, text="Value = 85.0")
scaleFunHeight.set(85)
...
labelFunRadiusValue = Label(frameFunRadius, text="Value = 5.0")
...
scaleTriAngle = Scale(frameTriAngle, variable=triangleAngle, from_=0, to=200, orient=HORIZONTAL, length=300)
buttonTriAngleValue = Button(frameTriAngle, text="Get Value", command=selTA)
labelTriAngleValue = Label(frameTriAngle, text="Value = 50.0 Angle = 90.0°")
